...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const zonConfig = config.vals[params.zon as keyof typeof config.vals];
  
  if (!zonConfig || !zonConfig.active) {
    throw new Response("Not Found", { status: 404 });
  }

  const files = await client.vals.files.retrieve(zonConfig.id, {
    path: "",
    recursive: true,
  }).then(res => res.data);
...
export default function ZonDetail() {
  const { files, zon, title } = useLoaderData<typeof loader>();
 
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-center mb-8">
        <GalleryIcon className="w-8 h-8 text-blue-600 mr-3" />
        <h1 className="text-3xl font-bold">Files in {title}</h1>
      </div>
